...
def __proc_np_hv(pred):
    ...
    if con_map:
        edges =  pred[...,-1]
        np.save("./rsc/edges.npy", pred[...,-1])
        print(np.unique(edges), np.max(edges), np.min(edges))
    # processing
    blb = np.array(blb_raw >= 0.5, dtype=np.int32)
    if con_map:
        edges = np.array(edges >= 0.5, dtype=np.float32)
        edges_norm = cv2.normalize(
        edges, None, alpha=0, beta=1, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_32F)
        print('normalized edges', np.unique(edges), np.max(edges), np.min(edges))
    

    blb = measurements.label(blb)[0]
    blb = remove_small_objects(blb, min_size=10)
    
    ##REmoving HOles(start)
    ...
    blb = measurements.label(blb)[0]
    out = remove_small_objects(blb, min_size=10)
    
    
    out[out > 0] = 1  # background is 0 already

    
    np.logical_not(out, out=blb)
    ##REmoving HOles (end)
