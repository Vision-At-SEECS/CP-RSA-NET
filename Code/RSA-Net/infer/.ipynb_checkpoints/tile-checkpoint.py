...
#             print('overlaid: ', overlaid_img.shape, cv2.cvtColor(overlaid_img, cv2.COLOR_RGB2BGR).shape)
        ...
        while len(file_path_list) > 0:
            ...
            while len(file_path_list) > 0:
                ...
                #############################################
                f_ext = file_path.split('.')[-1]
                print(file_path)
#                 print(f_ext)
                if (f_ext == 'png') | (f_ext=='tif'):
                    img = cv2.imread(file_path)
                    img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
                else:
                    img = np.load(file_path).astype('float32')
#                 print(img.shape)
                ##############################################
                src_shape = img.shape
...
#             print('cache_patch_info_list ', len(cache_patch_info_list), cache_patch_info_list)
            print('file_paths : ', len(file_paths), file_paths)
            ...
            accumulated_gradcam_output = []
            
            GC = False
            for batch_idx, batch_data in enumerate(dataloader):
#                 print(file_path[batch_idx])
                cache_image_info_list
                sample_data_list, sample_info_list = batch_data
#                 print(sample_data_list.size(), sample_info_list.size())
                if GC:
                    sample_output_list, grad_cams = self.run_step(sample_data_list)  
                    print(cache_image_info_list[file_idx])

                    if batch_idx >= file_sum + cache_image_info_list[file_idx][1]:
                        file_sum = cache_image_info_list[file_idx][1]
                        file_idx += 1


                    np.save(f"./grad_cam/consep/{file_paths[file_idx].split('.')[0]}_cam_img{batch_idx}.npy", grad_cams[1])
                    np.save(f"./grad_cam/consep/{file_paths[file_idx].split('.')[0]}_grayscale_image{batch_idx}.npy", grad_cams[0])
                    np.save(f"./grad_cam/consep/{file_paths[file_idx].split('.')[0]}_batch_{batch_idx}.npy" , sample_data_list[0,...])

                    np.save(f"./grad_cam/consep/{file_paths[file_idx].split('.')[0]}_prediction_{batch_idx}.npy" , sample_output_list[0,...])
                    print('pred ', sample_output_list.shape)
                else: 
                ...
                if GC:
                    ############### Grad Cam
                    sample_gradcam_list = list(zip(sample_info_list, grad_cams[1]))
                    accumulated_gradcam_output.extend(sample_gradcam_list)
                    ############### Grad Cam
                pbar.update()
            pbar.close()
            ...
            for file_idx, file_path in enumerate(use_path_list):
                ...
                if GC:
                    file_gc_data, accumulated_gradcam_output = detach_items_of_uid(
                       accumulated_gradcam_output, file_idx, image_info[1]
                    )
