...
#     print(pairwise_inter)
    ...
    if (pairwise_inter.shape[0] == 0) & (pairwise_union.shape[1] == 0): #caters for blank images , where gt and pred both are 0s
        print('Blank image was detected as blank image ')
        aji_score = 1 #since it wwas able to detect a blank image 
    elif (pairwise_inter.shape[0] == 0) | (pairwise_union.shape[1] == 0): #caters for images , where either gt or pred is 0
        print('Either the image was blank or the network wasnt able to predict any nuclei')
        aji_score = 0 #since there was a problem with detection 
    else:
...
def get_dice_1(true, pred):
    ...
    sum_denom = np.sum(denom)
    
    print('\n DICE SCORE: \n ','\t intersection: ', sum_inter, '\t denominator: ',  sum_denom)
    
    if (sum_inter == 0) & (sum_denom == 0): #caters for blank images , where gt and pred both are 0s
        print('Blank image was detected as blank image ')
        return 1.0 #since it wwas able to detect a blank image 
    
    elif (sum_inter == 0) | (sum_denom == 0): #caters for images , where either gt or pred is 0
        print('Either the image was blank or the network wasnt able to predict any nuclei')
        return 0.0 #since there was a problem with detection 
    
    else:
        return 2.0 * sum_inter / sum_denom
