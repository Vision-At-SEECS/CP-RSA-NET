...
Usage:
  run.py --true_path=<n> --pred_path=<n> --save_path=<n> --type_path=<n>
  run.py (-h | --help)
  run.py --version
...
Options:
  ...
  --type_path=<n>    Path where types of the GT are saved
"""
...
#     types_path = os.path.join(true_root,'types.npy') # path to the nuclei types
    ...
    for i in range(pred.shape[0]):
        print(f'\n {i}')
        pq = []
        pred_bin = remap_label(binarize(pred[i,:,:,:5]))
        true_bin = remap_label(binarize(true[i,:,:,:5]))
        print('pred bin(int32):', np.unique(pred_bin))
        print('true bin(int32):', np.unique(true_bin))
...
#             print('true types(int32) bef: (class ', j, ' )', np.unique(true_tmp))
            
#             print('true types(int32) bef: (class ', j, ' )', np.unique(pred_tmp))
            pred_tmp = remap_label(pred_tmp)
            true_tmp = remap_label(true_tmp)
            
            
#             print('pred types(int32): (class ', j, ' )', np.unique(pred_tmp))
#             print('true types(int32):(class ', j, ' )', np.unique(true_tmp))
...
#                 continue
    ...
    for tissue_name in tissue_types:
        indices = [i for i, x in enumerate(types[:pred.shape[0]]) if x == tissue_name]
