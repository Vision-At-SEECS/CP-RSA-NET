...
def run_nuclei_type_stat(pred_dir, true_dir, type_uid_list=None, exhaustive=True):
    ...
    for file_idx, filename in enumerate(file_list[:]):
        ...
        print(file_idx, basename)
        
        
        
        true_info = sio.loadmat(os.path.join(true_dir, basename.replace('image', 'mask') + ".mat"))
        print('shapes ', true_info['inst_type'].shape, true_info['inst_centroid'].shape, '\n\n')
#         if true_info['inst_type']
        # dont squeeze, may be 1 instance exist
        if len(true_info["inst_centroid"].shape) == 3:
            true_centroid = (true_info["inst_centroid"][0,:,:]).astype("float32")
        else:
            true_centroid = (true_info["inst_centroid"]).astype("float32")
            if -1 in true_centroid[0]:
                continue
        ...
        print('true_inst_Type: ', true_inst_type, true_inst_type.shape)
        ...
        print(true_centroid.shape, pred_centroid.shape)
    ...
    print('paired all: ', len(paired_all))
    print('true_inst_type all: ', len(true_inst_type_all))
    print(paired_all, true_inst_type_all)
    ...
    print(paired_all[:,0].shape)
    print('paired all: ', paired_all.shape, paired_all[:,0].shape, paired_all[:,1].shape)
    
    print(true_inst_type_all.shape)
