...
def train_step(batch_data, run_info):
    ...
    if con_map:
        true_con = batch_data["con_map"].to("cuda").type(torch.int64)
    ...
    if con_map:
        true_con_onehot = (F.one_hot(true_con, num_classes=2)).type(torch.float32)
        true_dict = {
            "np": true_np_onehot,
            "hv": true_hv,
            "con": true_con_onehot, #true_con,
            }
    ...
    if con_map:
        pred_dict["con"] = F.softmax(pred_dict["con"], dim=-1)

    if model.module.nr_types is not None:
        pred_dict["tp"] = F.softmax(pred_dict["tp"], dim=-1)
        
    
#     print(true_con.size(), pred_dict["con"].size(), pred_dict["np"].size())
    ####
    loss = 0
...
#             print(branch_name ,loss_name)
    ...
    if con_map:
        pred_dict["con"] = pred_dict["con"][..., 1]  # return pos only
    ...
    if con_map: 
        result_dict["raw"]["con"] = (true_dict["con"], pred_dict["con"])
...
def valid_step(batch_data, run_info):
    ...
    if con_map:
        true_con = torch.squeeze(true_con).to("cuda").type(torch.int64)
    ...
    with torch.no_grad():  # dont compute gradient
        ...
        if con_map:
            pred_dict["con"] = F.softmax(pred_dict["con"], dim=-1)[..., 1]
    ...
    if con_map:
        result_dict["raw"]["true_con"] = true_dict["con"].cpu().numpy()
        result_dict["raw"]["pred_con"] = pred_dict["con"].cpu().numpy()
...
#         print(name, ':', param.requires_grad)
    ...
    with torch.no_grad():  # dont compute gradient
        ...
        if con_map:
            ...
            del pred_dict["con"]
#             print(con.size())
            pred_dict["con"] = F.softmax(con, dim=-1)[..., 1:]
