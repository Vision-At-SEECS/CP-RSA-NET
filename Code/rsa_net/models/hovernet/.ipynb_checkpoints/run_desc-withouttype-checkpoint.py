...
#         TODO: Reduce the below bogus lengthy knowledge, that was written in a haste
        if idx==0:
            model_output_np = model_output['np'].unsqueeze(0)  
            model_output_hv =  model_output['hv'].unsqueeze(0)
            if "tp" in model_output:
                model_output_hv =  model_output['tp'].unsqueeze(0)
...
#         deaug_mask_hv2 = transformer.deaugment_mask(model_output_hv2)
    ...
    for img_aug in image_augs:
        ...
        augmented_image = img_aug(images=patch_images.cpu().numpy())
    ...
    mean_mask = {
        ...
        "hv": torch.cat((torch.tensor(np.maximum(model_output_hv1.cpu().numpy()[0,...], model_output_hv1.cpu().numpy()[0,...])).unsqueeze(1),
                         torch.tensor(np.maximum(model_output_hv2.cpu().numpy()[0,...], model_output_hv2.cpu().numpy()[0,...])).unsqueeze(1)), axis=1).cuda(),
    }
    print(mean_mask['hv'].size())
    return collections.OrderedDict(mean_mask)
