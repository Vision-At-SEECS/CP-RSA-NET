...
#         print(channels, kernel_size, 'cbam\'s kernel size')
        ...
        self.compress2 = nn.Conv1d(channels, 1, kernel_size=k_size, padding=(k_size - 1) // 2, bias=False)
#         self.spatial = BasicConv(2, 1, kernel_size, stride=1, padding=(kernel_size-1) // 2, relu=False)


#         self. = BasicConv(2, 1, kernel_size, stride=1, padding=(kernel_size-1) // 2, relu=False)    
        self.spatial = BasicConv(3, 1, kernel_size, stride=1, padding=(kernel_size-1) // 2, relu=False)
    def forward(self, x):
        x_compress = torch.cat((self.compress(x), self.compress2(x).unsqueeze(1)), dim=1)
