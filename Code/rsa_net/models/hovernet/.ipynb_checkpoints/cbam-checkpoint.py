...
#         self.bn = nn.BatchNorm1d(1,eps=1e-5, momentum=0.01, affine=True, track_running_stats=False) 
#         self.relu = nn.ReLU()
...
#         ch_att_sum = self.relu(y).unsqueeze(-1)
        ...
        y = self.conv(y.squeeze(-1).transpose(-1, -2)).transpose(-1, -2).unsqueeze(-1)
#         y = self.relu(self.bn(y.transpose(-1, -2)).transpose(-1, -2))
