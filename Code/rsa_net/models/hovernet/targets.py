...
def gen_instance_contour(ann):
    img =  ann.astype('uint8')                                                
    kernel = np.ones((3,3),np.uint8)                                          
    r= cv2.erode(img, kernel, iterations=1)                                   
                                                                                
    #Edge extraction                                                          
    con_map=img-r
#     print(np.unique(con_map), np.unique(np_map), np.unique(hv_map))
    con_map[con_map>0] = 1
    return con_map
...
def gen_instance_hv_map(ann, crop_shape):
    ...
    if cont:
        con_map = np.zeros(orig_ann.shape[:2], dtype=np.float32)
    ...
    for inst_id in inst_list:
        ...
        if cont:
            con_map += gen_instance_contour(inst_map)
...
def gen_targets(ann, crop_shape, **kwargs):
    ...
    cont = False
    if cont:
        hv_map, con_map = gen_instance_hv_map(ann, crop_shape)
    else:
    ...
    if cont:
        con_map = cropping_center(con_map, crop_shape)
    
#     np.save('con.npy', con_map)
    target_dict = {
        "hv_map": hv_map,
        "np_map": np_map,
    }
...
#         np_map[idx, 1, :, :] = e                                            
    if cont:                                                                           
        ...
        return target_dict
#     print("np map, con map", np_map.shape, hv_map.shape, con_map.shape)
#     print('\n \n np and hv map sizes ', np_map.shape, hv_map.shape)           
#     print(np.unique(np_map[:,:,0]), np.unique(np_map[:,:,1]), '\n \n ')       
